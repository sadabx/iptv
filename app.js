...
function loadChannel(id, streamIdx) {
  ...
  hideErr();
  startHLS(url);
  if (streamIdx == null) toast(`▶ ${ch.name}`);
}
        ...
        switch (data.type) {
          case Hls.ErrorTypes.NETWORK_ERROR:
            ...
            if (isManifestError && hasBackup) {
              ...
              retryCount++;
              toast(`⚠️ Network issue. Retrying (${retryCount}/${MAX_RETRIES})...`);
              hls.startLoad();
            } else {
              handleFatalError(data);
            }
          ...
          case Hls.ErrorTypes.MEDIA_ERROR:
            if (retryCount < MAX_RETRIES) {
              retryCount++;
              toast(`⚠️ Media issue. Recovering (${retryCount}/${MAX_RETRIES})...`);
...
function handleFatalError(data) {
  ...
  if (ch) {
    ...
    if (activeStreamIdx + 1 < streams.length) {
      ...
      toast(`⚠️ Stream failed. Switching to backup ${streams[nextIdx].label}...`);
      clearPlayTimeoutWatchdog();
      setTimeout(() => {
  ...
  playTimeoutTimer = setTimeout(() => {
    ...
    if ($video.paused || $video.seeking || !$video.currentTime) {
      ...
      if (ch) {
        ...
        if (activeStreamIdx + 1 < streams.length) {
          const nextIdx = activeStreamIdx + 1;
          toast(`⚠️ Load timeout. Switching to backup ${streams[nextIdx].label}...`);
